...
logger.debug("************"+j_data["unit_num"]+"  "+j_data["unit_name"]+" STS_RAWの書き込み START*******************")
logger.debug('DB接続OK')
#csvファイル読み込み  
...
for f in files:
    try:
        ...
        for index, row in df_in_2.iterrows():
            ...
            # try:
            sql = "SELECT * FROM IOT_DATA.STATUS_RAW WHERE EQP_ID="+ row2["EQP_ID"] +"AND OCCURRED_DT="+ row2["OCCURRED_DT"] + "AND STATUS_CONTENTS="+ row2['STATUS_CONTENTS'] 
            sql_rows = []
            ...
            if sql_rows:
                ...
                if not released_time == row["RELEASED_DT"]:
                    sql_update = "UPDATE IOT_DATA.STATUS_RAW SET RELEASED_DT="+ row2["RELEASED_DT"] +" WHERE EQP_ID="+ row2["EQP_ID"] +"AND OCCURRED_DT="+ row2["OCCURRED_DT"] + "AND STATUS_CONTENTS="+ row2['STATUS_CONTENTS'] 
                ...
                str_value = ','.join(row2)
                sql_insert = "INSERT INTO IOT_DATA.STATUS_RAW ("+ str_columnname +") values(" + str_value + ")" 
                logger.debug("SQL_insert:"+sql_insert)
                ibm_db.exec_immediate(conn,sql_insert)
    except:
        pass
logger.debug("************"+j_data["unit_num"]+"  "+j_data["unit_name"]+" STS_RAWの書き込み END*********************")